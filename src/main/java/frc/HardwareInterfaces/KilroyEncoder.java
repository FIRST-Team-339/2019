...
public class KilroyEncoder implements PIDSource {
    ...
    public int get() {
        ...
        case REV_CAN:
            // Spark Motor Controllers return an double based on the number of rotations it
            // has completed

            // sparkTicksPerRevolution is the number we multi the value of revolutions by to
            // get a larger integer imitating ticks
            if (canEncoder.getInverted() == true)
                return -(int) (this.sparkTicksPerRevolution * canEncoder.getEncoder().getPosition());
