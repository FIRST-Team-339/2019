...
public class Autonomous {
    ...
    public static enum State {
        INIT, DELAY, CHOOSE_PATH, CROSS_AUTOLINE, DEPOSIT_STRAIGHT_CARGO_HATCH, DEPOSIT_ROCKET_HATCH,
        DEPOSIT_SIDE_CARGO_HATCH, FINISH
    }
    ...
    public static void periodic() {
        if (Hardware.rightDriver.getRawButton(11) && Hardware.leftDriver.getRawButton(11)) {
            autoState = State.FINISH;
        }
    ...
    private static enum DepositCargoHatchState {
        INIT, DESCEND, TURN_1_RIGHT_SIDE, TURN_1_LEFT_SIDE, DRIVE_1, TURN_2_RIGHT_SIDE, TURN_2_LEFT_SIDE, DRIVE_2,
        ALIGN_TO_CARGO, DEPOSIT_CARGO
    }
    ...
    private static boolean depositCargoHatch() {
        ...
        case TURN_1_RIGHT_SIDE:
            if (Hardware.drive.turnDegrees(-TURN_90, DRIVE_SPEED, ACCELERATION_TIME, USING_GYRO)) {
        ...
        case TURN_1_LEFT_SIDE:
            ...
            break;
        case DRIVE_1:
            if (Hardware.drive.driveStraightInches(DRIVE_STRAIGHT_DEPOSIT_1, DRIVE_SPEED, ACCELERATION_TIME,
                    USING_GYRO)) {
                if (autoPosition == Position.RIGHT) {
        ...
        case TURN_2_LEFT_SIDE:
            ...
            break;
        case DRIVE_2:
            if (Hardware.drive.driveStraightInches(DRIVE_STRAIGHT_DEPOSIT_2, DRIVE_SPEED, ACCELERATION_TIME,
                    USING_GYRO)) {
        ...
        case ALIGN_TO_CARGO:
            ...
            if (Hardware.driveWithCamera.driveToTarget(DRIVE_WITH_CAMERA_SPEED)) {
                depositCargoHatchState = DepositCargoHatchState.DEPOSIT_CARGO;
            }
            break;
        case DEPOSIT_CARGO:
    ...
    private static enum RocketHatchState {
        STANDBY, DESCEND, STRAIGHTEN_OUT_ON_WALL, DRIVE_FORWARD_TO_TURN, TURN_TOWARDS_FIELD_WALL,
        DRIVE_TOWARDS_FIELD_WALL, TURN_ALONG_FIELD_WALL, ALIGN_PERPENDICULAR_TO_TAPE, DRIVE_TO_ROCKET_TAPE,
        ALIGN_TO_ROCKET, DEPOSIT_HATCH, FINISH, DRIVE_BY_CAMERA
    }
    ...
    private static boolean depositRocketHatch() {
        ...
        case DESCEND:
            ...
            if (usingVision == true) {
                ...
                autoTimer.reset();
                autoTimer.start();
                Hardware.drive.drive(DRIVE_AGAINST_WALL_SPEED, DRIVE_AGAINST_WALL_SPEED);
        ...
        case DRIVE_FORWARD_TO_TURN:
            if (Hardware.drive.driveStraightInches(DISTANCE_TO_DRIVE_TO_FIRST_TURN, DRIVE_SPEED, ACCELERATION_TIME,
                    true) == true) {
        ...
        case TURN_TOWARDS_FIELD_WALL:
            ...
            if (autoPosition == Position.RIGHT
                    && Hardware.drive.turnDegrees(TURN_RIGHT90, TURN_SPEED, ACCELERATION_TIME, true)) {
            ...
            // turn for if we are on the left side of th field
                    ...
                    && Hardware.drive.turnDegrees(TURN_LEFT90, TURN_SPEED, ACCELERATION_TIME, true)) {
                rocketHatchState = RocketHatchState.DRIVE_TOWARDS_FIELD_WALL;
            }
            break;
        case DRIVE_TOWARDS_FIELD_WALL:
            if (Hardware.frontUltraSonic.getDistanceFromNearestBumper() >= DISTANCE_NEEDED_TO_TURN) {
        ...
        case TURN_ALONG_FIELD_WALL:
            // turn for if we are on the right side of the field
            if (autoPosition == Position.RIGHT
                    && Hardware.drive.turnDegrees(TURN_LEFT90, TURN_SPEED, ACCELERATION_TIME, false)) {
                rocketHatchState = RocketHatchState.ALIGN_PERPENDICULAR_TO_TAPE;
            }
            // turn for if we are on the left side of th field
            else if (autoPosition == Position.LEFT
                    && Hardware.drive.turnDegrees(TURN_RIGHT90, TURN_SPEED, ACCELERATION_TIME, false)) {
                rocketHatchState = RocketHatchState.ALIGN_PERPENDICULAR_TO_TAPE;
            }
            break;
        ...
        case DRIVE_BY_CAMERA:
            ...
            case INIT:
                ...
                break;
            case DRIVE:
                if (Hardware.drive.driveStraightInches(DISTANCE_TO_CROSS_AUTOLINE, .6, ACCELERATION_TIME, USING_GYRO)) {
            ...
            case TURN_RIGHT:
                ...
                if (Hardware.drive.turnDegrees(TURN_FOR_CAMERA_DEGREES, .2, .2, USING_GYRO)) {
    ...
    private static enum SideCargoHatchState {
        INIT, LEAVE_LEVEL_2, TURN_AFTER_LEVEL_2_DROP, LEAVE_LEVEL_1_ONLY, DRIVE_1, TURN_1, DRIVE_2, TURN_2,
        DRIVE_TO_TAPE, DRIVE_AFTER_TAPE, TURN_AFTER_TAPE, DRIVE_TO_CARGO_SHIP, SCORE, FINISHED
    ...
    public static boolean driveOffStraightLevel1(LightSensor backIR1, LightSensor backIR2, Drive drive) {
    ...
    public static boolean descendFromLevelTwo(boolean usingAlignByWall) {
        ...
        case DRIVE_FAST:
            if (descentTimer.get() >= TIME_TO_DRIVE_OFF_PLATFORM) {
                ...
                // false);
                Hardware.transmission.driveRaw(SPEED_TO_DRIVE_OFF_PLATFORM, SPEED_TO_DRIVE_OFF_PLATFORM);
            }
            break;

        case LANDING_SETUP:

            if (Hardware.testRedLight.isOn() == true && usingAlignByWall == true) {
                descentState = DescentState.BACKWARDS_TIMER_INIT;
            } else if (Hardware.testRedLight.isOn() && usingAlignByWall == false) {
        ...
        case DRIVE_BACKWARDS_TO_ALIGN:
            if (descentTimer.get() >= TIME_TO_DRIVE_BACKWARDS_TO_ALIGN) {
                ...
                Hardware.transmission.driveRaw(DRIVE_BACKWARDS_SPEED, DRIVE_BACKWARDS_SPEED);
            }
            break;

        case FINISH:
            System.out.println("YEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEETTTTTTTTTTTTTTTTTTTT");
            return true;
